...
resource "aws_security_group" "frontend_sg" {
  ...
  ingress {
    ...
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"]
    description = "Permite tráfico HTTPS"
  }
  egress {
    from_port   = 0
    to_port     = 0
...
resource "aws_security_group" "backend_sg" {
  ...
  ingress {
    ...
    to_port     = 3000
    protocol    = "tcp"
    cidr_blocks = [aws_subnet.public_subnet_ftn.cidr_block, aws_subnet.public_subnet_jump.cidr_block] # Permite tráfico desde las subredes públicas (donde estará el frontend) - ¡AJUSTAR SI ES NECESARIO!
    description = "Permite tráfico desde el Frontend"
  }
  ingress {
    from_port   = 22
    to_port     = 22
    protocol    = "tcp"
    cidr_blocks = [aws_subnet.public_subnet_ftn.cidr_block, aws_subnet.public_subnet_jump.cidr_block] # Permite SSH desde las subredes públicas (para el jump server) - ¡AJUSTAR SI ES NECESARIO!
    description = "Permite SSH desde las subredes públicas (para el jump server)"
  }
  egress {
    from_port   = 0
    to_port     = 0
...
resource "aws_instance" "jump_server" {
  ...
  subnet_id     = aws_subnet.public_subnet_jump.id
  vpc_security_group_ids = [aws_security_group.jump_server_sg.id]
  key_name      = "tu-clave-ssh"       # Reemplaza con el nombre de tu clave SSH en AWS primero la tendremos que agregar
  tags = {
...
resource "aws_instance" "frontend_server" {
  ...
  subnet_id     = aws_subnet.public_subnet_ftn.id
  vpc_security_group_ids = [aws_security_group.frontend_sg.id]
  key_name      = "tu-clave-ssh" #Se usara la que se creo
  tags = {
...
resource "aws_instance" "backend_server" {
  ami           = "ami-0e449927258d45bc4" # Linux 2023
  instance_type = "t2.medium"
  subnet_id     = aws_subnet.private_subnet_bkd.id
  vpc_security_group_ids = [aws_security_group.backend_sg.id]
  key_name      = "tu-clave-ssh" // se creara una llave solamente para esta instancia.
  tags = {
